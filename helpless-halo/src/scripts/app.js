function todoApp() {
  ...
  this.changeMode = function (mode) {
    this.mode = mode;
    this.container.innerHTML = ""; /* Container leer machen */
    this.printButtons();
              ...
              <div class="row">
                <div class="col">
                  <div class="p-1 pt-3 mb-3">
                    <button id="new-todo" class="btn btn-sm btn-info">Nachricht eingeben</button>
                  </div>
                </div>
              </div>
            `;
  ...
  this.printTodo = function (todo) {
    ...
    let element = this.container.querySelector(
      `#todo-${todo.id}`
    ); /* selektiere dieses Element */
    element.addEventListener("change", (event) => {
      event.preventDefault(); // Verhindern das da standart mäßig was gemacht wird
      /* Daten zusammen sammeln */
      let data = {
        id: todo.id,
        completed: event.target.checked ? 1 : 0, // wenn abgehackt dann ist 1, wenn nicht abgehackt dann 0.
      };
      /* Funktion aufrufen und übergebe Daten! */
    ...
    let html = `
            <div class="row">
              <div class="col">
                <form class="p-1">
                  <div class="mb-3">
                    <label for="todo-title" class="form-label">Name</label>
                    <input type="text" class="form-control" id="todo-title" value="${
                      this.currentTodo.name
                    }">
                  </div>
                  <div class="mb-3">
                    <label for="todo-description" class="form-label">Nachricht</label>
  ...
  this.getTodo = function (todo) {
    return this.apiHandler(
      `http://localhost:8888/api/todo/${todo.id}`,
      "GET"
    ).then((json) => {
      return json;
    });
  ...
  this.setTodo = function (todo) {
    return this.apiHandler("http://localhost:8888/api/todo", "POST", todo).then(
      (json) => {
        return json;
      }
    );
  ...
  /* Es wird eine URL gebraucht, method ist http Methode und Datensatz standartmäßig ist immer die Null */
  this.apiHandler = function (url, method, data = null) {
    let params = {}; // Parameter, wenn was drinnen steht
    ...
    /* Nach Return kann ich dann immer noch was machen */
    return fetch(url, {
      method,
      cache: "no-cache",
      ...params, // Value wird integriert mit diesen ... Punkten
    })
      .then((response) => {
